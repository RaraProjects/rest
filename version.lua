...
local apiURL             = 'https://api.github.com/repos/RaraProjects/rest/releases'
local mainBranchURL      = 'https://github.com/RaraProjects/rest/archive/refs/heads/main.zip'
...
local uiGetGitHubData = function()
    if versionDataPulled == false then
        ...
        UI.Text('- Check Github for recent Rest releases.')
        UI.Text('- Likely cause a very short system stutter.')
        UI.Text('- Show you a table of Rest versions (in game).')
        UI.Text('- NOT download files to your computer.')
        UI.Text('')

        if UI.Button('Check for new Rest releases.') then
...
local basicInstallInstructions = function()
    ...
    UI.Text('2. Move your rest.zip file to addons folder.')
    UI.Text('3. Unzip rest.zip.')
    UI.Text('4. You can overwrite the rest files.')
end
...
local uiInstructions = function()
    ...
    elseif hasTestRelease then
        ...
        UI.Text('6. Leave a comment in the Rest Discord or Github Issues page.')
    end
end
